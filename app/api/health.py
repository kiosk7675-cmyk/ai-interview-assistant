...
async def health_check():
    ...
    try:
        ...
        chroma_message: str = (
            "ChromaDB 连接正常" if chroma_healthy else "ChromaDB 连接异常"
        )
        health_data["vector_db"] = {
            "type": "chromadb",
    ...
    # 如果向量数据库不可用，服务标记为有问题但不503（ChromaDB 本地文件，启动时可能未初始化）
    if health_data.get("vector_db", {}).get("status") not in ("connected",):
